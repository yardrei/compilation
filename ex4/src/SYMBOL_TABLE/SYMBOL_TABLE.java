...
public class SYMBOL_TABLE {
    ...
    private int variable_caunter = 0;
    ...
    /****************************************************************/
    /* Enter a variable to the symbol table with scope and location */
    /****************************************************************/
    public void enter(String name, TYPE t, scopeEnum scope, int location) {
        int hashValue = hash(name);
        SYMBOL_TABLE_ENTRY next = table[hashValue];
        // Prepare a new symbol table entry with name, type, next, prevtop, clope and location
        SYMBOL_TABLE_ENTRY entry = new SYMBOL_TABLE_ENTRY(name, t, hashValue, next, top, top_index++, scope, location);
        top = entry;
        table[hashValue] = entry;
        PrintMe();
    }
    ...
    public TYPE find(String name) {
        for (SYMBOL_TABLE_ENTRY entry = table[hash(name)]; entry != null; entry = entry.next) {
            if (name.equals(entry.name)) {
                System.out.println("type ID(SYMBOL_TABLE.find): " + name + "==" + entry.name);////////////////////
                System.out.println("type name(SYMBOL_TABLE.find): " + entry.type.name);////////////////////
                return entry.type;
            }
        }
    ...
    public void setLastFunctionType(TYPE functionType) {
    ...
    public void setLastFunctionName(String name) {
    ...
    public void setLastClassName(String name) {
        lastClassName = name;
    }

    /*************************************************/
    /** Gives a variable his location in a function **/
    /*************************************************/
    public int getNewVariableIndex() {
        variable_caunter += 1;
        return variable_caunter;
    }

    //Resets the variable counter
